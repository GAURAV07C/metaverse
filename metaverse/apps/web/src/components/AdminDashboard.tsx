...
interface Element { id: string; imageUrl: string; width: number; height: number; static: boolean }
interface Avatar { id: string; imageUrl: string; name: string }
...
export function AdminDashboard() {
  ...
  const createElement = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setElImageUrl(''); setElWidth('1'); setElHeight('1'); setElStatic(true);
      await fetchElements();
    } catch (err: any) {
  ...
  const handleUpdateElement = async () => {
    ...
    try {
      ...
      await api.put(`/admin/element/${editingElementId}`, { imageUrl: editImageUrl });
  ...
  const createMap = async (e: React.FormEvent) => {
    e.preventDefault();
    setMapError(''); setMapSuccess(''); setMapLoading(true);
    try {
  ...
  return (
    <div className="dashboard">
      ...
      <aside className="sidebar glass">
        <div className="sidebar-logo">🛡️ <span>Admin Panel</span></div>
      ...
      <main className="dash-main">
        ...
        {tab === 'elements' && (
          <div className="animate-fade-in">
            <div className="dash-header"><h1>🪑 Elements</h1></div>

            {/* Create form */}
            <div className="glass admin-form-card">
              <h3 style={{ marginBottom: '1.25rem', fontWeight: 600 }}>
                <Plus size={16} style={{ display: 'inline', marginRight: 6 }} /> Create Element
              </h3>
              <form onSubmit={createElement} className="admin-grid-form">
                {elError && <div className="error-banner" style={{ gridColumn: '1/-1' }}>{elError}</div>}
                <div className="field">
                  <label className="field-label">Image URL</label>
                  <input className="input" value={elImageUrl} onChange={e => setElImageUrl(e.target.value)} placeholder="https://..." />
                </div>
                <div className="field">
                  <label className="field-label">Width (tiles)</label>
                  <input className="input" type="number" value={elWidth} onChange={e => setElWidth(e.target.value)} min={1} />
                </div>
                <div className="field">
                  <label className="field-label">Height (tiles)</label>
                  <input className="input" type="number" value={elHeight} onChange={e => setElHeight(e.target.value)} min={1} />
                </div>
                <div className="field" style={{ justifyContent: 'flex-end' }}>
                  <label className="field-label">Static?</label>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginTop: '0.5rem' }}>
                    <input type="checkbox" checked={elStatic} onChange={e => setElStatic(e.target.checked)} style={{ width: 18, height: 18, accentColor: 'var(--accent)', cursor: 'pointer' }} />
                    <span style={{ color: 'var(--text-secondary)', fontSize: '0.85rem' }}>Non-walkable</span>
                  </div>
                </div>
                <button type="submit" className="btn" style={{ gridColumn: '1/-1' }} disabled={elLoading}>
                  {elLoading ? <span className="spinner" /> : 'Create Element →'}
                </button>
              </form>
            </div>
              ...
              {elements.map(el => (
                <div key={el.id} className="glass admin-item-card">
                  <div className="admin-item-thumb">
                    <img src={el.imageUrl} alt="" onError={(e) => { (e.target as HTMLImageElement).src = 'https://placehold.co/60x60/1e293b/94a3b8?text=?'; }} />
                  </div>
            ...
            {/* Edit Element Modal */}
            {editingElementId && (
              <div className="modal-overlay" onClick={() => setEditingElementId(null)}>
                <div className="modal glass animate-fade-in" onClick={e => e.stopPropagation()}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
                    <h2>Update Element Image</h2>
                    <button className="btn-icon" onClick={() => setEditingElementId(null)}><X size={18} /></button>
                  </div>
                  ...
                  <div style={{ display: 'flex', gap: '0.75rem' }}>
                    <button className="btn btn-ghost btn-full" onClick={() => setEditingElementId(null)}>Cancel</button>
                    <button className="btn btn-full" onClick={handleUpdateElement} disabled={updateLoading}>
        ...
        {tab === 'maps' && (
          <div className="animate-fade-in">
            <div className="dash-header"><h1>🗺️ Maps</h1></div>
            <div className="glass admin-form-card">
              <h3 style={{ marginBottom: '1.25rem', fontWeight: 600 }}>
                <Plus size={16} style={{ display: 'inline', marginRight: 6 }} /> Create Map
              </h3>
              <form onSubmit={createMap} className="admin-grid-form">
                {mapError && <div className="error-banner" style={{ gridColumn: '1/-1' }}>{mapError}</div>}
                {mapSuccess && <div className="success-banner" style={{ gridColumn: '1/-1' }}>{mapSuccess}</div>}
                <div className="field">
                  <label className="field-label">Map Name</label>
                  <input className="input" value={mapName} onChange={e => setMapName(e.target.value)} placeholder="e.g. Office Room" />
                </div>
                <div className="field">
                  <label className="field-label">Dimensions (WxH)</label>
                  <input className="input" value={mapDims} onChange={e => setMapDims(e.target.value)} placeholder="100x200" />
                </div>
                <div className="field" style={{ gridColumn: '1/-1' }}>
                  <label className="field-label">Thumbnail URL</label>
                  <input className="input" value={mapThumb} onChange={e => setMapThumb(e.target.value)} placeholder="https://thumbnail.com/a.png" />
                </div>
                ...
                <div style={{ gridColumn: '1/-1', borderTop: '1px solid var(--glass-border)', paddingTop: '1rem', marginTop: '0.5rem' }}>
                  ...
                  {elements.length > 0 && (
                    <div className="map-el-builder">
                      <select className="input" value={addElId} onChange={e => setAddElId(e.target.value)} style={{ flex: 2 }}>
                        <option value="">Select element…</option>
                        {elements.map(el => (
                          <option key={el.id} value={el.id}>
                            {el.id.slice(0, 8)}… ({el.width}×{el.height}, {el.static ? 'static' : 'walkable'})
                          </option>
                        ))}
                      </select>
                      <input className="input" type="number" placeholder="X" value={addElX} onChange={e => setAddElX(e.target.value)} style={{ flex: 1 }} />
                      <input className="input" type="number" placeholder="Y" value={addElY} onChange={e => setAddElY(e.target.value)} style={{ flex: 1 }} />
                      <button type="button" className="btn" style={{ padding: '0.5rem 1rem' }} onClick={addMapElement} disabled={!addElId}>
                        <Plus size={14} />
                      </button>
                    </div>
                  )}
                  {elements.length === 0 && (
                    <p style={{ color: 'var(--text-secondary)', fontSize: '0.8rem' }}>Create elements first (Elements tab)</p>
                  )}
                    ...
                    <div className="map-el-list">
                      {mapDefaultElements.map((mel, i) => (
                        <div key={i} className="map-el-chip">
                          ...
                          <button type="button" onClick={() => removeMapElement(i)} style={{ background: 'none', border: 'none', color: 'var(--danger)', cursor: 'pointer', padding: 0 }}>
                            <X size={12} />
                          </button>
                        </div>
                      ))}
                ...
                <button type="submit" className="btn" style={{ gridColumn: '1/-1' }} disabled={mapLoading}>
                  {mapLoading ? <span className="spinner" /> : `Create Map with ${mapDefaultElements.length} element${mapDefaultElements.length !== 1 ? 's' : ''} →`}
                </button>
              </form>
            </div>
        ...
        {tab === 'avatars' && (
          <div className="animate-fade-in">
            <div className="dash-header"><h1>🧑‍🤝‍🧑 Avatars</h1></div>
            <div className="glass admin-form-card">
              <h3 style={{ marginBottom: '1.25rem', fontWeight: 600 }}>
                <Plus size={16} style={{ display: 'inline', marginRight: 6 }} /> Create Avatar
              </h3>
              <form onSubmit={createAvatar} className="admin-grid-form">
                {avError && <div className="error-banner" style={{ gridColumn: '1/-1' }}>{avError}</div>}
                <div className="field">
                  <label className="field-label">Avatar Name</label>
                  <input className="input" value={avName} onChange={e => setAvName(e.target.value)} placeholder="e.g. Timmy" />
                </div>
                <div className="field" style={{ gridColumn: '1/-1' }}>
                  <label className="field-label">Image URL</label>
                  <input className="input" value={avImageUrl} onChange={e => setAvImageUrl(e.target.value)} placeholder="https://..." />
                </div>
                <button type="submit" className="btn" style={{ gridColumn: '1/-1' }} disabled={avLoading}>
                  {avLoading ? <span className="spinner" /> : 'Create Avatar →'}
                </button>
              </form>
            </div>

            <h3 style={{ margin: '2rem 0 1rem', color: 'var(--text-secondary)' }}>All Avatars ({avatars.length})</h3>
            <div className="admin-items-grid">
              {avatars.map(av => (
